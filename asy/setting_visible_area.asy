...
shipout("rotate_frame1",frame1, BBox(2mm,Draw(black)));

// Дублирование по горизонтали
for (int i = 1; i < 2; ++i){
...
for (int i = 0; i < snowV.length; ++i) {
  path snow = T*graph(snowV[i])--cycle;
  // draw(pic_iter, snow, brown+linewidth(2));
  fill(pic, snow, orange+opacity(0.8));
  // string labelText = "$" + string(i + 1) + "$";
  // string labelText = "$(" + string((snowV[i][0]).x)+"; " + string(((snowV[i][0])).y)+ ")$";
  // label(pic,scale(0.2)*labelText, T*(snowV[i][0]), (0.5,0.5),blue);
  // }
  // label(pic,labelText, T*(snowV[i][0]), (0.5,0.5), purple);
}
shipout("duplication_pattern", pic, BBox(2mm,Draw(black)));

// real xmin = 2.85*(24*3^(iterations-2) +6*(iterations - 1)); 
// real xmax = 3.6*(24*3^(iterations-2) +6*(iterations - 1));
// real ymin = ((1.5*6 - 2.5)*3^(iterations - 2));
// real ymax = ((3.4*6 - 2.5)*3^(iterations - 2));
...
write("xmin = " + string(xmin, 2) + ", xmax = " + string(xmax, 2) + ", ymin = " + string(ymin, 2) + ", ymax = " + string(ymax, 2));
write("xmax - xmin = " + string(xmax-xmin, 2));
write("ymax - ymin = " + string(ymax-ymin, 2));
write("(xmax - xmin)/(ymax - ymin) = " + string((xmax-xmin)/(ymax-ymin), 2));
...
// Построение осевого прямоугольника
pair Aprime = (B.x, A.y);
pair Bprime = (B.x, C.y);
pair Cprime = (D.x, C.y);
pair Dprime = (D.x, A.y);
...
// clip(pic,viewRectD);
shipout("visiable_area", pic, BBox(2mm, Draw(black)));
...
//   // if ( i % 16 == 1){
//   a.add(pic);
//   // shipout(format("frames/frame_%04d", i), pic, BBox(2mm,Fill(white)));
//   // }
// }
